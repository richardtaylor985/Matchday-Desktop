...
async function initialiseWindowsIntegrationSettings() {
  if (!window.matchdayWindows) return;
  const host = document.querySelector("#settingsModal .settings-body, #settingsModal .modal-body, #settingsModal .settings-content");
  if (!host || document.getElementById("windowsIntegrationSettings")) return;

  const state = await window.matchdayWindows.getSettings().catch(() => ({}));
  const section = document.createElement("section");
  section.id = "windowsIntegrationSettings";
  section.className = "settings-section";
  section.innerHTML = `
    <h3>WINDOWS INTEGRATION</h3>
    <label class="settings-check">
      <input id="useMatchdayScreensaver" type="checkbox" ${state.useScreenSaver !== false ? "checked" : ""}>
      <span>Set Matchday Desktop as my Windows screensaver</span>
    </label>
    <label class="settings-check">
      <input id="startMatchdayWindows" type="checkbox" ${state.startWithWindows ? "checked" : ""}>
      <span>Start Matchday Desktop automatically with Windows</span>
    </label>
    <label class="settings-field">
      <span>Screensaver starts after</span>
      <select id="matchdayScreenSaverTimeout">
        ${[300,600,900,1800].map(v => `<option value="${v}" ${(state.timeoutSeconds || 600) === v ? "selected" : ""}>${v/60} minutes</option>`).join("")}
      </select>
    </label>
    <button id="saveWindowsIntegration" type="button">SAVE WINDOWS SETTINGS</button>
    <div id="windowsIntegrationStatus" aria-live="polite"></div>
  `;
  host.appendChild(section);

  document.getElementById("saveWindowsIntegration").addEventListener("click", async () => {
    const status = document.getElementById("windowsIntegrationStatus");
    status.textContent = "Saving…";
    try {
      await window.matchdayWindows.saveSettings({
        useScreenSaver: document.getElementById("useMatchdayScreensaver").checked,
        startWithWindows: document.getElementById("startMatchdayWindows").checked,
        timeoutSeconds: Number(document.getElementById("matchdayScreenSaverTimeout").value)
      });
      status.textContent = "Windows settings saved.";
    } catch (error) {
      status.textContent = `Could not save Windows settings: ${error.message || error}`;
    }
  });
}

const _matchdayOriginalOpenSettings = typeof openSettings === "function" ? openSettings : null;
if (_matchdayOriginalOpenSettings) {
  openSettings = function(...args) {
    const result = _matchdayOriginalOpenSettings.apply(this, args);
    setTimeout(initialiseWindowsIntegrationSettings, 0);
    return result;
  };
}
